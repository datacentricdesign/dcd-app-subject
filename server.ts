...
import { Strategy, RouterAPI, PersonService } from '@datacentricdesign/sdk-js';
...
const fs = require('fs');
const template = fs
  .readFileSync(join(DIST_FOLDER, 'browser', 'subject', 'index.html'))
  .toString();
...
passport.use(
  'oauth2',
  new Strategy(strategyOptions, (accessToken, refreshToken, profile, cb) =>
...
app.get(baseUrl + '/', checkAuthentication, async (req, res, next) => {
  console.log('baseUrl');
...
app.get(baseUrl + '/page/*', checkAuthentication, async (req, res, next) => {
  console.log('page');
  res.render('index', { req });
});
...
// Recup mapsKey
app.get(baseUrl + '/mapsKey', checkAuthentication, (req, res) => {
  console.log('mapsKey');
  res.send({ key: google_maps_key });
});
...
app.listen(PORT, () => {
  console.log(
    `Node Express server listening on http://localhost:${PORT}${baseUrl}`
  );
});
